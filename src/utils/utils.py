...
def extract_predict_annotate(output_dir,ensemble_models,video_glob,video_idxs, transformer,blazeface_dir,device,model_list):
    ...
    for ne in tqdm(video_idxs, desc='Annotating videos'):
        ...
        while success:
            ...
            cv2.putText(img, 'Fake Frames ' + str(fake_frames), (int(w/2)+int(w/6),  int(h/2) +int(h/10)   ), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
    ...
    print("Annotate videos saved to "+output_dir)
